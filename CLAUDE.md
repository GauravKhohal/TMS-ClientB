# TMS (Transport Management System) — Agent Guide — Client B

**This is a separate, standalone deployment forked from the base TMS-1 codebase for a
specific client.** It has its own database, hosting (Railway/Vercel), and JWT secret —
fully isolated from TMS-1 and any other client's deployment. Features here are expected
to diverge (different modules/workflows) — do not assume parity with TMS-1.
